...
export const InitUtilsCtx = (model:Model)=>{
    currentModel = model;
}
    ...
    for (let i = 0; i < scanLines; i++) {
        ...
        if (AET) {
            ...
            for (j = 0; j < arr.length; j++) {
                ...
                DrawLineByBresenham(imgData,new Point(x0,y),new Point(x1, y),vertsArray,tNormalsArray,vertsUV,color);
            }
            arr = [];
        }
    }
